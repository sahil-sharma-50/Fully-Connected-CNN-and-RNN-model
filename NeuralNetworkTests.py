...
class TestNeuralNetwork3(unittest.TestCase):
    ...
    def test_iris_data_with_momentum(self):
        ...
        if TestNeuralNetwork3.plot:
            ...
            plt.plot(net.loss, '-x')
            fig.savefig(os.path.join(self.directory, "TestNeuralNetwork3_Momentum.pdf"), transparent=True, bbox_inches='tight',
                        pad_inches=0)
    ...
    def test_iris_data_with_batchnorm(self):
        ...
        if TestNeuralNetwork3.plot:
            ...
            plt.plot(net.loss, '-x')
            fig.savefig(os.path.join(self.directory, "TestNeuralNetwork3_Batchnorm.pdf"), transparent=True, bbox_inches='tight',
                        pad_inches=0)
    ...
    def _perform_test(self, optimizer, iterations, description, dropout, batch_norm):
        ...
        accuracy = Helpers.calculate_accuracy(results, labels)
        with open(self.log, 'a') as f:
            print('On the UCI ML hand-written digits dataset using {} we achieve an accuracy of: {}%'.format(description, accuracy * 100.),
                  file=f)
